...
def discover_experiments(root: Path = Path(".")) -> DiscoveryResult:
    ...
    try:
        ...
        for current_text, directory_names, file_names in walker:
            ...
            if _CONFIGURATION_FILE in names or _STATUS_FILE in names:
...
def _load_candidate(
    ...
    configuration_path = candidate / _CONFIGURATION_FILE
    status_path = candidate / _STATUS_FILE
    if not configuration_path.is_file():
        return None, DiscoveryDiagnostic(candidate, f"missing {_CONFIGURATION_FILE}")
    if not status_path.is_file():
        return None, DiscoveryDiagnostic(candidate, f"missing {_STATUS_FILE}")
    try:
        configuration = ExperimentConfiguration.from_dict(load_json_object(configuration_path))
        status = ExperimentStatus.from_dict(load_json_object(status_path))
